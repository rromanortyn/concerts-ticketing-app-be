...
class CityController {
  ...
  async getCities(): Promise<GetCitiesResponseDto[]> {
    const { data } = await this.queryBus.execute(new GetCitiesQuery())

    return plainToInstance(GetCitiesResponseDto, data, {
  ...
  async addCity(@Body() dto: AddCityRequestDto): Promise<AddCityResponseDto> {
    const { data } = await this.commandBus.execute(new AddCityCommand(dto))

    return plainToInstance(AddCityResponseDto, data, {
      excludeExtraneousValues: true,
    })
  }
}
